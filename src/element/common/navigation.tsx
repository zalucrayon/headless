...
export default function Navigation() {
  ...
  const [localizedSlugs, setLocalizedSlugs] = useState<any>(null);

  useEffect(() => {
    const handleUpdate = () => {
      if (typeof window !== 'undefined') {
        setLocalizedSlugs((window as any).__localized_slugs || null);
      }
    };
    window.addEventListener('localizedSlugsUpdated', handleUpdate);
    // Check initial value
    handleUpdate();
    return () => {
      window.removeEventListener('localizedSlugsUpdated', handleUpdate);
    };
  }, []);
  ...
  const updateConfig = async (url: any) => {
    ...
    if (config?.langmenu?.show_menu) {
      ...
      const rawMenu = config?.menu[_mlang?.typo3Language] || [];
      setcurrentMenu(rawMenu);
      config.activelang = _mlang;
      setActivelang(_mlang);
    } else {
      const rawMenu = config?.menu[0] || [];
      setcurrentMenu(rawMenu);
      config.activelang = 0;
      setActivelang(0);
    }
  ...
  const getLangugeUrl = (_mn: any) => {
    ...
    let resolvedPath = _mn?.path;
    let isDetailMatched = false;

    // Check if we are on a dynamic detail page
    const segments = url.split('/').filter(Boolean);
    const langCodes = config?.langmenu?.langm?.map((l: any) => {
      return l?.path?.split('/')[1]?.toLowerCase();
    }).filter(Boolean) || [];

    let actualSegments = [...segments];
    if (segments.length > 0 && langCodes.includes(segments[0].toLowerCase())) {
      actualSegments = segments.slice(1);
    }

    if (actualSegments.length >= 2) {
      const postSlug = actualSegments[actualSegments.length - 1];
      const parentPath = '/' + actualSegments.slice(0, actualSegments.length - 1).join('/');

      // Find translation for parent/container page
      const containerPage = config?.pages?.find((pg: any) => {
        const pgSlugWithoutPrefix = pg?.slug?.replace(/^\/[a-z]{2}\//i, "/")?.replace(/\/+$/, "") || "/";
        const normParent = parentPath.replace(/\/+$/, "") || "/";
        return pgSlugWithoutPrefix === normParent;
      });

      if (containerPage) {
        const parentPageUid = containerPage.sys_language_uid === 0 ? containerPage.uid : containerPage.l10n_parent;
        let targetPage = null;
        if (_mn?.uid === 0) {
          targetPage = config?.pages?.find((pg: any) => {
            return pg?.uid === parentPageUid && pg?.sys_language_uid === 0;
          });
        } else {
          targetPage = config?.pages?.find((pg: any) => {
            return pg?.l10n_parent === parentPageUid && pg?.sys_language_uid === _mn?.uid;
          });
        }
        ...
        if (targetPage) {
          ...
          if (localizedSlugs) {
            ...
            if (localizedSlugs[targetLangId]) {
              targetPostSlug = localizedSlugs[targetLangId].slug || localizedSlugs[targetLangId];
            }
          }

          resolvedPath = `${baseSlug}/${targetPostSlug}`;
          isDetailMatched = true;
        }
      }
    }
    ...
    if (!isDetailMatched) {
      ...
      if (_m) {
        ...
        if (_fdat) {
          resolvedPath = _fdat?.slugurl;
        } else {
          resolvedPath = _mn?.path;
        }
      } else {
        let _pagedata = config?.pages?.find((pg: any) => {
          return pg?.slug === url;
